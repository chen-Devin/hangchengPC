    ...
    var search = function (fn) {
        form.on('submit(search)', function (data) {
            ...
            if (isNaN(begin) || isNaN(end)) {
                ...
                table.reload("dataReload", { where: whereOptions });
            }
        });
    }
    ...
    var initPage = {
        ...
        'doarkRecord': function () {
            setSelect($('select[name="doarkTime"]'));
             ...
             //获取客户数据
             bizUtil.layui.loadDataToSelect('#customerName');   
            ...
            var getColumns = (function () {
                ...
                tableColumns = [
                    ...
                    { field: 'carNo', title: '车牌', width: 150, align: 'center', sort: true },
                    { field: 'carAtt', title: '车属性', width: 100, align: 'center', sort: true }
        ...
        'dolistCustomer': function() {    
            ...
            form.on('select(dateType)', function (data) {
                if(data.value == 1){
                    ...
                    renderTable(getColumns('year'), getWhereParam());
                }else if(data.value == 2){
                    setSelect($('select[name="statDate"]'));
                    $('input[name="hasPlace"]').prop('disabled', false);       
                    
                    var hasPlace = $('input[name="hasPlace"]')[0].checked === true ? 'monthHasPlace' : 'month';
                    renderTable(getColumns(hasPlace), getWhereParam());
                }else{
            ...
            var fancySearch = function(filter, type) {
                form.on(type + '(' + filter + ')', function (data) {
                    var isYear = $('select[name="dateType"]').val();
                    var hasPlace = $('input[name="hasPlace"]')[0].checked === true ? 'monthHasPlace' : 'month';        
                    renderTable(getColumns(isYear == '1' ? 'year' : hasPlace), getWhereParam());
                });              
            }

            fancySearch('statDate', 'select');
            fancySearch('customerId', 'select');
            fancySearch('hasPlace', 'checkbox');
            ...
            var getColumns = function (type) {
                ...
                var makeColumns = function(hasPlace, timeColumns) {
                    var columns = [
                        { field: 'id', title: '序号', width: 80, align: 'center', templet: '#idTpl' },
                        { field: 'customer', title: '客户', width: 150, align: 'center'}
                    ];
                    if(hasPlace){
                        columns = columns.concat({ field: 'place', title: '装卸地点', width: 150, align: 'center'});
                    }
            ...
            var getWhereParam = function(){
                var whereOptions = {
                    'dateType': $('select[name="dateType"]').val(),
                    'statDate': $('select[name="statDate"]').val(),
                    'customerId': $('select[name="customerId"]').val(),
                    'hasPlace': $('input[name="hasPlace"]')[0].checked
                }
                return whereOptions;
            };

            function renderTable(columns, where) {
                var options = {
                    "tableOptions": {
                        cols: [columns],
            ...
            renderTable(getColumns('month'), getWhereParam());
